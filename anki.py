...
def_deck = genanki.Deck(
  2059400110,
  f'{droppedFile.stem}-prepsheet')
def_deck.add_model(def_model)


reg = r"\\boxset\{(.*?)\}\n\{([\w\W]*?)\}%endboxset\n"

with open(droppedFile) as file:
  ...
  for find in finds:
    ...
    my_note = genanki.Note(
              ...
              guid=f"{droppedFile.stem}-prepsheet.tex-{name}",
              tags=["prepsheet",droppedFile.stem]
              )
    def_deck.add_note(my_note)
  genanki.Package(def_deck).write_to_file(cwd / f"{droppedFile.stem}_prepsheet.apkg")
